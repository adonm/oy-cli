...
- 2026-04-03: Finished the remaining provider protocol hotspot split by replacing `internal/oy/providers/protocol.go` with focused files (`protocol_messages.go`, `protocol_decode.go`, `protocol_reasoning.go`, `protocol_helpers.go`), preserved the flat provider package API, and refreshed the audit tracker so M1 is now resolved.
