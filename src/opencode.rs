...
"#,
    ...
    fn generated_audit_and_review_agents_document_chunk_failure_recovery() {
        ...
        assert!(OY_AUDITOR_AGENT.contains("tool_output.max_bytes = 256 KiB"));
        ...
        assert!(OY_REVIEWER_AGENT.contains("tool_output.max_bytes = 256 KiB"));
    }
...
- `oy setup` writes `tool_output.max_bytes = 256 KiB` to opencode config so a default-sized chunk (DEFAULT_TARGET_TOKENS = 64000 at ~4 chars/token) fits in one tool result. If you raise `target_tokens` above 64000 (e.g. for a single oversized file), a full chunk may exceed opencode's tool-output cap and be truncated to a preview; in that case prefer narrower `path` values over larger `target_tokens` when possible.
