...
fn section_after_heading<'a>(report: &'a str, heading: &str) -> Option<&'a str> {
    ...
    let body = rest.get(..body_end).unwrap_or(rest).trim();
    (!body.is_empty()).then_some(body)
}

fn parse_heading_findings(source: FindingSource, text: &str, level: usize) -> Vec<Finding> {
    let mut findings = Vec::new();
...
fn parse_summary_bullets(source: FindingSource, text: &str) -> Vec<Finding> {
    text.lines()
        .filter_map(|line| {
            let trimmed = line.trim();
            let body = trimmed
