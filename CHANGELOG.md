...
### Changed
- **OpenCode 2 stable:** follow the released OpenCode 2 `latest` channel everywhere. The installer now provisions `npm:@opencode/cli` through mise with the up-front tool options its install needs (`allow_builds` for the binary-selecting postinstall, `allow_low_downloads` for the newly published package name), installs `node@latest` for that postinstall, migrates `npm:@opencode-ai/cli` beta entries to the stable package, and no longer prints a manual `allow_builds` snippet. Host compatibility accepts tagged `2.x.y` builds only: beta/next channel handling is removed, and unsupported hosts just skip the optional post-setup location refresh.
...
- Closed each Cursor tool progress summary before waiting for its result so
  ...
  lifecycles and abort session draining.
