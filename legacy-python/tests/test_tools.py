...
from tests.conftest import DummyHttpClient, make_state, patch_runtime, tool_handler
...
class TestToolSchemas:
    def test_closed_object_schemas(self):
        ...
        assert (
            specs["todo"]["parameters"]["properties"]["todos"]["items"][
...
class TestToolApproval:
    def test_validates_arguments_and_respects_approval_modes(
...
class TestAskTodoTools:
    def test_ask_and_todo_update_state(self, monkeypatch, tmp_path):
        ...
        assert rendered == {
            "items": [{"id": "t1", "task": "ship it", "status": "in_progress"}],
            "count": 1,
        }
...
class TestBashTool:
    def test_summarizes_json_and_text(self, monkeypatch, tmp_path):
        ...
        results = iter(
            [
                SimpleNamespace(
                    returncode=0, stdout='{"ok":true,"items":[1,2]}', stderr=""
                ),
...
class TestFileTools:
    def test_small_helper_functions(self, tmp_path):
        ...
        assert (
            tools._existing_tool_target(tmp_path, "exists.txt", tool="read") == target
        )
    ...
    def test_read_stops_after_requested_slice(self, tmp_path, monkeypatch):
        ...
        state = make_state(tmp_path)
        patch_runtime(monkeypatch, show=None, note_tool=None)
    ...
    def test_list_read_search_replace_sloc(self, tmp_path, monkeypatch):
        ...
        excluded_payload = tools.tool_search(
            state, "alpha|hello", ".", exclude=["dir/**", "*.zip"], limit=20
        )
        ...
        replace_excluded = tools.tool_replace(
            state, "alpha", "OMEGA", ".", exclude=["skip.txt"], limit=20
        )
...
class TestWebfetch:
    ...
    def test_webfetch_html_to_markdown_and_redaction(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(rt, "show", shown.append)
        monkeypatch.setattr(
            rt, "tool_session", lambda **kw: DummyHttpClient(response=response, **kw)
        )
    ...
    def test_webfetch_restricted_methods_and_headers(self, monkeypatch, tmp_path):
        ...
        for kwargs in (
            ...
            with pytest.raises(ValueError):
                tools.tool_webfetch(state, url="https://example.com/page", **kwargs)
