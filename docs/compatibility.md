...
oy 0.13.2 integrates with OpenCode 2 through its noninteractive runner, managed model API, version-matched npm plugin, and temporary local stdio MCP adapter. It defaults to the `opencode2` executable; `OY_OPENCODE` can override the executable but not the required contract.
